;(function(undefined) {
  ...
  function Noverlap() {
    ...
    this.atomicGo = function () {
      ...
      var margX = self.config.margX,
          margY = self.config.margY,
          scaleX = self.config.scaleX,
          scaleY = self.config.scaleY,
          gridX = self.config.gridX,
          gridY = self.config.gridY
      ;
      ...
      for (i=0; i < nodesCount; i++) {
        ...
        xmin = Math.min(xmin, n.dn_x - (n.dn_width*scaleX + margX) );
        xmax = Math.max(xmax, n.dn_x + (n.dn_width*scaleX + margX) );
        ymin = Math.min(ymin, n.dn_y - (n.dn_height*scaleY + margY) );
        ymax = Math.max(ymax, n.dn_y + (n.dn_height*scaleY + margY) );
      ...
      for (i=0; i < nodesCount; i++) {
        ...
        nxmin = n.dn_x - (n.dn_width*scaleX + margX);
        nxmax = n.dn_x + (n.dn_width*scaleX + margX);
        nymin = n.dn_y - (n.dn_height*scaleY + margY);
        nymax = n.dn_y + (n.dn_height*scaleY + margY);

        minXBox = Math.floor(gridX * (nxmin - xmin) / (xmax - xmin) );
        maxXBox = Math.floor(gridX * (nxmax - xmin) / (xmax - xmin) );
        minYBox = Math.floor(gridY * (nymin - ymin) / (ymax - ymin) );
        maxYBox = Math.floor(gridY * (nymax - ymin) / (ymax - ymin) );
      ...
      for(row = 0; row < gridY; row++) {
        for(col = 0; col < gridX; col++) {
          grid[row][col].forEach(function(nodeId) {
            ...
            for(subRow = Math.max(0, row - 1); subRow <= Math.min(row + 1, gridY - 1); subRow++) {
              for(subCol = Math.max(0, col - 1); subCol <= Math.min(col + 1, gridX - 1); subCol++) {
      ...
      for (i=0; i < nodesCount; i++) {
        ...
        adjacentNodes[n1.id].forEach(function(nodeId) {
          ...
          var collision = false;
          if (Math.abs(xDist) * 2 < ((n1.dn_width / 2 * scaleX) + (n2.dn_width / 2 * scaleX) + margX) ) collision = true;
          if (Math.abs(yDist) < ((n1.dn_height / 2 * scaleY) + (n2.dn_height / 2 * scaleY) + margY) ) collision = true;
          var dist = Math.sqrt(xDist*xDist + yDist*yDist);
          if(collision) {
            self.running = true;
            if(dist > 0) {
              n2.dn.dx += xDist / dist * (1 + n1.dn_width);
              n2.dn.dy += yDist / dist * (1 + n1.dn_height);
            } else {
    ...
    this.start = function() {
      ...
      var labelSizeRatio = this.sigInst.settings('labelSizeRatio');
