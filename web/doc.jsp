...
String title = "";
if (doc != null)
    title = ML.detag(doc.doc().get("scope"));
...
<body class="document">
    <header>
        ...
        <form class="search" id="search" autocomplete="off" action="#"
            role="search">
            <!-- 
       ...
       -->
            <%=selectCorpus(alix.name)%>, <label for="titles">Chercher
                un titre</label> <input id="titles" name="titles"
                aria-describedby="titles-hint"
                placeholder="am… dia… eu… fed…" size="50" />
            <div class="progress">
                <div></div>
            </div>
            <div class="suggest"></div>
            <input type="hidden" id="id" name="id" value="<%=id%>"
                autocomplete="off" size="13" /> <label>Mots
                clés :</label> <label for="cat">Filtrer</label> <select
                name="cat" onchange="this.form.submit()">
                <option></option>
                <%=pars.cat.options()%>
            </select> <label for="distrib">Score</label> <select name="distrib"
                onchange="this.form.submit()">
                <option></option>
                <%=pars.distrib.options()%>
            </select> <label for="q">Surligner</label> <input id="q" name="q"
...
if (doc != null) {
    ...
    continue;
        out.print("<a title=\"score : " + formatScore(forms.score()) + "\" href=\"?id=" + id + "&amp;q="
        + JspTools.escape(form) + "\" class=\"form\">");
