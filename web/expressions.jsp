...
<body>
    <header>
        ...
        <form class="search">
            ...
            <br /> <label>Score</label> <select name="mi"
                onchange="this.form.submit()">
                <option />
                <%=mi.options()%>
            </select> <label for="locs"
                title="Montrer les locutions connues du dictionnaire d’indexation">Locutions</label>
            <input name="locs" type="checkbox"
                <%=(locs) ? "checked" : ""%> onchange="this.form.submit()" />
            <label for="floor">Fréquence minimale</label> <input
                name="floor" size="1" class="num3"
                value="<%=(floor > 0) ? floor : ""%>" /> <label
                for="parceque">Mots vides</label> <input name="parceque"
                type="checkbox" <%=(parceque) ? "checked" : ""%>
                onchange="this.form.submit()" />
        </form>
    </header>
    <main>
        <table class="sortable">
            <caption>
                <details>
                    ...
                    <p>Ce tableau montre les expressions fréquentes.
                        Il est produit à partir d’un index des mots
                        fléchis (sans lemmatisation, sans dictionnaire
                        de locutions).</p>
                </details>
            </caption>
            <thead>
